...
[![PyPI version](https://img.shields.io/pypi/v/openregister.svg)](https://pypi.org/project/openregister/)
...
pip install git+ssh://git@github.com/stainless-sdks/openregister-python.git
```

> [!NOTE]
> Once this package is [published to PyPI](https://app.stainless.com/docs/guides/publish), this will become: `pip install --pre openregister`
...
async def main() -> None:
    response = await client.search.find_companies()
    print(response.results)
...
client.with_options(max_retries=5).search.find_companies()
```
...
client.with_options(timeout=5.0).search.find_companies()
```
...
search = response.parse()  # get the object that `search.find_companies()` would have returned
print(search.results)
```

These methods return an [`APIResponse`](https://github.com/stainless-sdks/openregister-python/tree/main/src/openregister/_response.py) object.

The async client returns an [`AsyncAPIResponse`](https://github.com/stainless-sdks/openregister-python/tree/main/src/openregister/_response.py) with the same structure, the only difference being `await`able methods for reading the response content.
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/openregister-python/issues) with questions, bugs, or suggestions.
