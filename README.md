...
async def main() -> None:
    response = await client.search.autocomplete_companies_v1(
        query="query",
    )
    print(response.results)
...
try:
    client.search.autocomplete_companies_v1(
        query="query",
    )
...
client.with_options(max_retries=5).search.autocomplete_companies_v1(
    query="query",
)
```
...
client.with_options(timeout=5.0).search.autocomplete_companies_v1(
    query="query",
)
```
...
search = response.parse()  # get the object that `search.autocomplete_companies_v1()` would have returned
